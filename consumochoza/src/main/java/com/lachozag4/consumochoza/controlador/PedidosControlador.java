...
import com.lachozag4.consumochoza.modelo.dto.PedidoDetalleDTO;
...
import com.lachozag4.consumochoza.service.IClienteService;
import com.lachozag4.consumochoza.service.IMesaService;
...
import com.lachozag4.consumochoza.service.ICuentaService;
...
public class PedidosControlador {
    ...
    public String listar(
        ...
        // Delegar filtrado, ordenamiento y paginaciÃ³n al servicio (SRP)
        PedidosPaginadosDTO resultado = pedidoService.listarConFiltros(estado, q, desde, hasta, page, size);

        // LÃ³gica especÃ­fica de la vista: determinar quÃ© pedidos muestran botÃ³n Cobrar
        ...
        // Determinar por cada combinaciÃ³n mesa-cliente un Ãºnico pedido con botÃ³n "Cobrar"
     ...
     * reutilizable tanto en el GET /nuevo como cuando hay errores de validaciÃ³n
     * en el POST /guardar.
     */
    private void prepararModeloPos(Model model, Authentication authentication, PedidoDTO pedido) {
        // Obtener el usuario logueado y asignarlo automÃ¡ticamente si no viene en el pedido
    ...
    public String guardar(@ModelAttribute PedidoDTO pedido,
        ...
        // Solo para pedidos NUEVOS. En ediciÃ³n, el backend ya se encarga de
        // restaurar el stock anterior y descontar el nuevo, por lo que no
        // aplicamos validaciÃ³n adicional aquÃ­.
        ...
        try {
            if (pedido.getIdpedido() == 0) {
                ...
                try {
                    ...
                    // Si el usuario seleccionÃ³ explÃ­citamente una cuenta existente, intentamos usarla
                    ...
                    // Si no hay cuenta seleccionada vÃ¡lida, aplicamos la lÃ³gica anterior
                    if (cuenta == null) {
                        if (crearNuevaCuenta) {
        ...
        // Ã‰xito: para nuevos pedidos mostramos mensaje en el POS, para ediciones en la lista
        if (pedido.getIdpedido() == 0) {
    ...
    public String mostrarFormularioEditar(@PathVariable int id, Model model, RedirectAttributes redirectAttributes) {
        ...
        // Bloquear ediciÃ³n si ya estÃ¡ en cocina, listo para entrega o en estado final
        if ("EN_COCINA".equals(pedido.getEstado())) {
            redirectAttributes.addFlashAttribute("mensajeError", "No se puede editar el pedido porque ya estÃ¡ en cocina");
            return "redirect:/pedidos";
        }
        if ("LISTO_PARA_ENTREGA".equals(pedido.getEstado())) {
            redirectAttributes.addFlashAttribute("mensajeError", "No se puede editar el pedido porque ya estÃ¡ listo para entrega");
            return "redirect:/pedidos";
        }
        ...
        // Asegura que la mesa asignada estÃ© en la lista aunque estÃ© ocupada
