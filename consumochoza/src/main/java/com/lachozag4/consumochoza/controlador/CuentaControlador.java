...
import com.lachozag4.consumochoza.modelo.dto.CambiarPasswordDTO;
...
public class CuentaControlador {
    ...
    public String cambiarPassword(@ModelAttribute("form") CambiarPasswordDTO form,
        ...
        if (form.getPasswordNuevo() == null || !form.getPasswordNuevo().equals(form.getPasswordConfirmacion())) {
            model.addAttribute("mensajeError", "La nueva contraseÃ±a y la confirmaciÃ³n no coinciden.");
            return "Usuario/CambiarPassword";
        }

        try {
            usuarioService.cambiarPassword(username, form.getPasswordActual(), form.getPasswordNuevo());
            model.addAttribute("mensajeExito", "ContraseÃ±a actualizada correctamente.");
            // Limpiar campos del formulario
            model.addAttribute("form", new CambiarPasswordDTO());
        } catch (Exception e) {
            model.addAttribute("mensajeError", e.getMessage() != null ? e.getMessage() : "Error al cambiar la contraseÃ±a.");
        }
