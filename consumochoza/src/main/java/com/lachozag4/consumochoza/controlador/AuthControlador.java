...
public class AuthControlador {
    ...
    @GetMapping("/login")
    public String login() {
        // Si no existe ningÃºn usuario en el sistema, redirigimos al
        // asistente de creaciÃ³n de administrador inicial.
    ...
    public String registrarAdminInicial(@ModelAttribute("usuario") UsuarioDTO usuario, Model model) {
        // Si ya se creÃ³ alguien entre medias, mandamos a login
        if (usuarioService.existeAlgunUsuario()) {
            return "redirect:/login";
        }
        try {
